...
import com.util.config.InfoMgr;
...
@Slf4j
public class ValidationMgrUtils {
    //邮箱表达式
...
//^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$
    ...
    public static boolean isIDCard(String IDCardNo) {
        ...
        if (IDCardNo.length() != 15 && IDCardNo.length() != 18) {
            ...
            log.error("AppValidationMgr-->>isIDCard", errmsg);
            return false;
        }
        ...
        try {
            ...
            e.printStackTrace();
            errmsg = "身份证生日不在有效范围";
            log.error("AppValidationMgr-->>isIDCard", errmsg  + e.getMessage());
            return false;
        } catch (java.text.ParseException e1) {
            e1.printStackTrace();
            errmsg = "身份证生日不在有效范围";
            log.error("AppValidationMgr-->>isIDCard", errmsg + e1.getMessage());
            return false;
        }
        ...
        //================ 地区码时候有效 ================
        Hashtable hashtable = InfoMgr.getAreaCodeAll();
...
//	}
    ...
    public static boolean getDateIsTrue(String year, String month, String day){
        try {
            ...
            e.printStackTrace();
            log.error("AppSysDateMgr-->>getDateIsTrue", e.getMessage().toString());
            return false;
        }
        return true;
    }
